"""
Lighter WebSocket and Market Maker client implementation.
"""
...
# Import custom WebSocket implementation and price manager
from .lighter_custom_websocket import LighterCustomWebSocketManager
from .lighter_price_manager import LighterPriceManager, PriceData
...
class LighterWSSMaker(BaseExchangeClient):
    """Lighter WebSocket and Market Maker client implementation."""

    def __init__(self, config: Dict[str, Any]):
        """Initialize Lighter WSS Maker client."""
        ...
        self.price_manager = LighterPriceManager(config)
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # Connect price manager first to get price data
    ...
    async def disconnect(self) -> None:
        ...
        try:
            # Disconnect price manager
            await self.price_manager.disconnect()
    ...
    async def fetch_bbo_prices(self, contract_id: str) -> Tuple[Decimal, Decimal]:
        """Get orderbook using price manager."""
        try:
            return await self.price_manager.get_bbo_prices()
        except ValueError as e:
            self.logger.log(f"Unable to get bid/ask prices: {e}", "ERROR")
            raise
    ...
    async def get_order_price(self, side: str = '') -> Decimal:
        """Get the price of an order using price manager."""
        # Get current market prices
        best_bid, best_ask = await self.price_manager.get_bbo_prices()
    ...
    # Price manager access methods
    def get_price_manager(self) -> LighterPriceManager:
        """Get the price manager instance."""
        return self.price_manager

    async def get_mid_price(self) -> Decimal:
        """Get mid price from price manager."""
        return await self.price_manager.get_mid_price()

    async def get_spread(self) -> Decimal:
        """Get spread from price manager."""
        return await self.price_manager.get_spread()

    def add_price_update_callback(self, callback) -> None:
        """Add price update callback to price manager."""
        self.price_manager.add_price_update_callback(callback)

    def remove_price_update_callback(self, callback) -> None:
        """Remove price update callback from price manager."""
        self.price_manager.remove_price_update_callback(callback)