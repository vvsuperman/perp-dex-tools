...
def _get_unique_library_path(original_path: str, instance_id: str) -> str:
    """
    为每个 SignerClient 创建一个独立的库副本
    """
    with _cache_lock:
        if instance_id not in _library_cache:
            # 创建临时目录
            temp_dir = tempfile.mkdtemp(prefix="lighter_signer_")
            # 复制库文件
            lib_name = Path(original_path).name
            unique_path = os.path.join(temp_dir, lib_name)
            shutil.copy2(original_path, unique_path)
            # 注册清理
            atexit.register(shutil.rmtree, temp_dir, ignore_errors=True)
            _library_cache[instance_id] = unique_path
        return _library_cache[instance_id]
...
def _initialize_signer_with_copy():
        _instance_id = str(uuid.uuid4())
         # 2. 获取原始库路径
        current_dir = os.path.dirname(os.path.abspath(__file__))
        signers_dir = os.path.join(current_dir, "signers")
        
        if platform.system() == "Darwin" and platform.machine().lower() == "arm64":
            original_lib = os.path.join(signers_dir, "signer-arm64.dylib")
        elif platform.system() == "Linux" and platform.machine().lower() in ("x86_64", "amd64"):
            original_lib = os.path.join(signers_dir, "signer-amd64.so")
        else:
            raise RuntimeError("Unsupported platform")

        # 3. 获取独立的库副本路径
        unique_lib_path = _get_unique_library_path(original_lib, _instance_id)

        # 4. 加载独立的 CDLL 实例
        logging.debug("copy signer library to and create CDLL instance")
        return ctypes.CDLL(unique_lib_path)
...
def create_api_key(seed=""):
    signer = _initialize_signer_with_copy()
...
class SignerClient:
    ...
    def __init__(
        ...
        self.signer = _initialize_signer_with_copy()
    ...
    async def create_order(
        ...
        logging.info(f"Create Order Tx Info: {tx_info}")

        api_response = await self.send_tx(tx_type=self.TX_TYPE_CREATE_ORDER, tx_info=tx_info)
        logging.info(f"Create Order Send Tx Response: {api_response}")
