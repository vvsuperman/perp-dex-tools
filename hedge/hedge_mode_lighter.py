...
class Lighter_Service:
    ...
    def __init__(self,  config: dict):
        self.lighter_market_index =  config['lighter_market_index']
        self.base_amount_multiplier = config['self.base_amount_multiplier']
        self.price_multiplier = config['self.price_multiplier'] 
        ...
        # Setup logger
        self.logger = logging.getLogger(f"hedge_bot")
...
class Hedge_Bot:
    ...
    async def trading_loop(self):
        ...
        try:
            ...
            # Get contract info
            config['lighter_market_index'], config['self.base_amount_multiplier'], config['self.price_multiplier'] = self.get_lighter_market_config()

            lighter_service1 = Lighter_Service(config)

            self.logger.info(f"Lighter: {self.lighter_market_index}")
    ...
    def get_lighter_market_config(self, ticker) -> Tuple[int, int, int]:
